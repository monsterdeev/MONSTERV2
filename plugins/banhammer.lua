...
local function kick_ban_res(extra, success, result) 
     ...
     if success == 0 then 
      ...
      local from_name = extra.from_name 
      ...
      elseif get_cmd == 'ban' then 
        ...
        send_large_msg(receiver, 'User @'..member..' ['..member_id..'] banned by '..from_name) 
      ban_user(member_id, chat_id) 
      elseif get_cmd == 'unban' then 
        send_large_msg(receiver, 'User @'..member..' ['..member_id..'] unbanned by '..from_name) 
        local hash =  'banned:'..chat_id 
        redis:srem(hash, member_id) 
        return 'User '..user_id..' unbanned' 
      elseif get_cmd == 'sban' then 
        send_large_msg(receiver, 'User @'..member..' ['..member_id..'] globally banned by '..from_name) 
      banall_user(member_id) 
      elseif get_cmd == 'unsban' then 
        send_large_msg(receiver, 'User @'..member..' ['..member_id..'] globally unbanned by '..from_name) 
 ...
 if matches[1]:lower() == 'id' and msg.to.type == "chat" or msg.to.type == "user" then 
  ...
  if matches[1]:lower() == 'ban' then-- /ban 
    ...
    elseif string.match(matches[2], '^%d+$') then 
      ...
      send_large_msg(receiver, 'User ['..matches[2]..'] banned by '..('@'..msg.from.username or msg.from.print_name)) 
      else 
      local cbres_extra = { 
      chat_id = msg.to.id, 
      get_cmd = 'ban', 
      ...
      chat_type = msg.to.type, 
                            from_name = ('@'..msg.from.username or msg.from.print_name) 
      } 
